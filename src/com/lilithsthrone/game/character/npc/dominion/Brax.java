...
class Brax extends NPC {
	...
	Brax(boolean isImported) {
		super(isImported,
		      ...
		      PlaceType.ENFORCER_HQ_BRAXS_OFFICE,
		      true);

		if (!isImported) {
			this.setAttribute(Attribute.MAJOR_CORRUPTION, 45);
			...
			AbstractItem potion = EnchantingUtils.craftItem(
					...
					tfPotion.getEffects().stream().map(
							x -> x.getEffect()).collect(
									Collectors.toList()));
	...
	void loadFromXML(
			...
			Document doc,
			CharacterImportSetting... settings) {
		loadNPCVariablesFromXML(this, null, parentElement, doc, settings);

		if (!this.isSlave() &&
		    Main.isVersionOlderThan(Main.VERSION_NUMBER, "0.2.10.5")) {
		...
		if (Main.isVersionOlderThan(Game.loadingVersion, "0.3.7.7") &&
			...
			TransformativePotion tfPotion = this.generateTransformativePotion(this); // Generate effects based on Brax's body
			AbstractItem potion = EnchantingUtils.craftItem(
					Main.game.getItemGen()
							.generateItem(tfPotion.getItemType()),
					tfPotion.getEffects()
							.stream()
							.map(x -> x.getEffect())
	...
	@Override
	public
	void setupPerks(boolean autoSelectPerks) {
		if (this.isSlave()) {
		...
		PerkManager.initialisePerks(this, Util.newArrayListOfValues(Perk.ENCHANTMENT_STABILITY, Perk.UNARMED_DAMAGE, Perk.ARCANE_BOOST), Util.newHashMapOfValues(new Value<>(PerkCategory.PHYSICAL, 2), new Value<>(PerkCategory.LUST, 0), new Value<>(PerkCategory.ARCANE, 1)));
	}

	@Override
	public
	void setStartingBody(boolean setPersona) {
		...
		if (setPersona) {
			...
			this.setFetishDesire(Fetish.FETISH_VAGINAL_GIVING,
			                     FetishDesire.THREE_LIKE);
		...
		// Coverings:
		this.setEyeCovering(new Covering(
				BodyCoveringType.EYE_LYCAN,
				PresetColour.EYE_YELLOW));
		this.setSkinCovering(new Covering(
				BodyCoveringType.LYCAN_FUR,
				PresetColour.COVERING_GREY),
		                     true);
		this.setSkinCovering(new Covering(
				BodyCoveringType.HUMAN,
				PresetColour.SKIN_OLIVE), true);

		this.setHairCovering(new Covering(
				BodyCoveringType.HAIR_LYCAN_FUR,
				PresetColour.COVERING_GREY),
		                     true);
		...
		this.setHairCovering(new Covering(
				...
				PresetColour.COVERING_BLACK), false);
	...
	@Override
	public
	void equipClothing(List<EquipClothingSetting> settings) {

		this.unequipAllClothingIntoVoid(true, true);

		if (Main.game.getPlayer()
				    .hasQuest(QuestLine.MAIN) &&
		    Main.game.getPlayer()
				    .isQuestProgressGreaterThan(QuestLine.MAIN, Quest.MAIN_1_C_WOLFS_DEN)) {
			AbstractClothing collar = Main.game.getItemGen()
					                          .generateClothing("innoxia_bdsm_metal_collar", false);
		...
		if (isFeminine()) {
			if (hasFetish(Fetish.FETISH_BIMBO)) {
				this.equipClothingFromNowhere(Main.game.getItemGen()
						                              .generateClothing("innoxia_groin_crotchless_panties", PresetColour.CLOTHING_PINK, false), true, this);
				this.equipClothingFromNowhere(Main.game.getItemGen()
						                              .generateClothing("innoxia_nipple_tape_crosses", PresetColour.CLOTHING_PINK, false), true, this);
				this.equipClothingFromNowhere(Main.game.getItemGen()
						                              .generateClothing(ClothingType.TORSO_FISHNET_TOP, PresetColour.CLOTHING_WHITE, false), true, this);
				this.equipClothingFromNowhere(Main.game.getItemGen()
						                              .generateClothing("innoxia_leg_micro_skirt_pleated", PresetColour.CLOTHING_PINK, false), true, this);
				this.equipClothingFromNowhere(Main.game.getItemGen()
						                              .generateClothing("innoxia_sock_fishnets", PresetColour.CLOTHING_WHITE, false), true, this);
				this.equipClothingFromNowhere(Main.game.getItemGen()
						                              .generateClothing("innoxia_hand_fishnet_gloves", PresetColour.CLOTHING_WHITE, false), true, this);
				this.equipClothingFromNowhere(Main.game.getItemGen()
						                              .generateClothing("innoxia_finger_ring", PresetColour.CLOTHING_GOLD, false), true, this);
				this.equipClothingFromNowhere(Main.game.getItemGen()
						                              .generateClothing(ClothingType.WRIST_BANGLE, PresetColour.CLOTHING_GOLD, false), true, this);

				this.setPiercedEar(true);
				this.equipClothingFromNowhere(Main.game.getItemGen()
						                              .generateClothing("innoxia_piercing_ear_hoops", PresetColour.CLOTHING_GOLD, false), true, this);
				this.setPiercedNose(true);
				this.equipClothingFromNowhere(Main.game.getItemGen()
						                              .generateClothing("innoxia_piercing_nose_ring", PresetColour.CLOTHING_GOLD, false), true, this);
				this.setPiercedNavel(true);
				this.equipClothingFromNowhere(Main.game.getItemGen()
						                              .generateClothing("innoxia_piercing_gemstone_barbell", PresetColour.CLOTHING_GOLD, false), InventorySlot.PIERCING_STOMACH, true, this);

			}
			else {
				this.equipClothingFromNowhere(Main.game.getItemGen()
						                              .generateClothing("innoxia_groin_thong", PresetColour.CLOTHING_WHITE, false), true, this);
				this.equipClothingFromNowhere(Main.game.getItemGen()
						                              .generateClothing("innoxia_chest_plunge_bra", PresetColour.CLOTHING_WHITE, false), true, this);
				this.equipClothingFromNowhere(Main.game.getItemGen()
						                              .generateClothing("innoxia_sock_kneehigh_socks", PresetColour.CLOTHING_WHITE, false), true, this);

				this.equipClothingFromNowhere(Main.game.getItemGen()
						                              .generateClothing("dsg_eep_servequipset_enfskirt", PresetColour.CLOTHING_BLACK, false), true, this);
				this.equipClothingFromNowhere(Main.game.getItemGen()
						                              .generateClothing("dsg_eep_ptrlequipset_flsldshirt", PresetColour.CLOTHING_PINK, false), true, this);
				this.equipClothingFromNowhere(Main.game.getItemGen()
						                              .generateClothing("innoxia_neck_tie", PresetColour.CLOTHING_BLACK, false), true, this);
			}

		}
		else {
			this.equipClothingFromNowhere(Main.game.getItemGen()
					                              .generateClothing("innoxia_groin_boxers", PresetColour.CLOTHING_BLACK, false), true, this);
			this.equipClothingFromNowhere(Main.game.getItemGen()
					                              .generateClothing(ClothingType.WRIST_MENS_WATCH, PresetColour.CLOTHING_STEEL, false), true, this);

			this.equipClothingFromNowhere(Main.game.getItemGen()
					                              .generateClothing("dsg_eep_servequipset_enfdslacks", PresetColour.CLOTHING_BLACK, false), true, this);
			this.equipClothingFromNowhere(Main.game.getItemGen()
					                              .generateClothing("dsg_eep_ptrlequipset_ssldshirt", PresetColour.CLOTHING_BLUE, false), true, this);
			this.equipClothingFromNowhere(Main.game.getItemGen()
					                              .generateClothing("innoxia_neck_tie", PresetColour.CLOTHING_BLACK, false), true, this);
			...
			if (!Main.game.isStarted() ||
				...
				if (settings.contains(EquipClothingSetting.ADD_WEAPONS)) {
					this.equipMainWeaponFromNowhere(Main.game.getItemGen()
							                                .generateWeapon("innoxia_crystal_epic", DamageType.FIRE));
					this.setEssenceCount(150);
					this.equipOffhandWeaponFromNowhere(Main.game.getItemGen()
							                                   .generateWeapon(WeaponType.getWeaponTypeFromId("dsg_eep_pbweap_pbpistol")));
				}
			}
		}
	}
	...
	String getDescription() {
		if (this.isSlave() &&
			...
			if (Main.game.getDialogueFlags()
					    .hasFlag(DialogueFlagValue.bimbofiedBrax)) {
			...
			else if (Main.game.getDialogueFlags()
					         .hasFlag(DialogueFlagValue.feminisedBrax)) {
	...
	void turnUpdate() {
		if (!this.isSlave() &&
		    !Main.game.getCharactersPresent()
				     .contains(this)) {
			this.setEssenceCount(150);
		}
		if (this.isSlave() &&
	...
	@Override
	public
	boolean isAbleToBeImpregnated() {
		return true;
	}

	@Override
	public
	boolean isImmuneToLevelDrain() {
		return !this.isSlave();
	}

	@Override
	public
	SexPace getSexPaceDomPreference() {
		if (this.isSlave() &&
		    this.getOwner()
				    .isPlayer()) {
	...
	@Override
	public
	SexPace getSexPaceSubPreference(GameCharacter character) {
		if (this.isSlave() &&
		    this.getOwner()
				    .isPlayer()) {
	...
	@Override
	public
	String getSpeechColour() {
		if (Main.getProperties()
				    .hasValue(PropertyValue.lightTheme)) {
			if (this.isFeminine()) {
				if (Main.game.getDialogueFlags().values.contains(DialogueFlagValue.bimbofiedBrax)) {
					return "#FF0AA5";
				}
	...
	@Override
	public
	void changeFurryLevel() {
	}

	@Override
	public
	DialogueNode getEncounterDialogue() {
		return null;
	}
	...
	Response endCombat(
		...
		if (victory) {
			return new Response("", "", BraxOffice.AFTER_COMBAT_VICTORY) {
				...
				public
				void effects() {
					if (Main.game.getPlayer().getQuest(
							QuestLine.MAIN) == Quest.MAIN_1_C_WOLFS_DEN) {
						Main.game.getTextEndStringBuilder()
								.append(Main.game.getPlayer()
										        .setQuestProgress(QuestLine.MAIN, Quest.MAIN_1_D_SLAVERY));
					}
				}
			};
	...
	public
	List<AbstractCoreItem> getLootItems() {
		return null;
	}

	@Override
	public
	int getLootEssenceDrops() {
		return 8;
	}

	@Override
	public
	TransformativePotion generateTransformativePotion(GameCharacter target) {
		...
		switch (Main.getProperties()
				        .getForcedTFPreference()) {
			case MAXIMUM:
			case NORMAL:
				maximumEffects.add(new PossibleItemEffect(
						new ItemEffect(
				...
				if (Main.getProperties().multiBreasts >
				    0) {
					if (target.getBreastRows() <
					    3) {
	...
	private static
	List<PossibleItemEffect> getFeminineEffects(
			GameCharacter target,
			AbstractItemType itemType) {
		List<PossibleItemEffect> effects = new ArrayList<>();

		for (int i = target.getFemininityValue();
		     i <
		     Femininity.FEMININE_STRONG.getMinimumFemininity();
		     i += 15) { // Turn feminine:
		...
		if (target.getMuscleValue() >
		    Muscle.THREE_MUSCULAR.getMedianValue()) {
		...
		if (target.getBodySizeValue() >
		    BodySize.TWO_AVERAGE.getMinimumValue()) {
		...
		for (int i = target.getBreastSize()
				             .getMeasurement();
		     i <
		     CupSize.E.getMeasurement();
		     i += 3) {
		...
		if (target.getHipSize()
				    .getValue() <
		    HipSize.FOUR_WOMANLY.getValue()) {
		...
		if (target.getAssSize()
				    .getValue() <
		    AssSize.FOUR_LARGE.getValue()) {
		...
		if (target.getHairRawLengthValue() >
		    0) { // If bald, leave bald.
			for (int i = target.getHairRawLengthValue();
			     i <
			     HairLength.THREE_SHOULDER_LENGTH.getMaximumValue();
			     i += 15) {
		...
		for (int i = target.getLipSizeValue();
		     i <
		     LipSize.TWO_FULL.getValue();
		     i += 2) {
